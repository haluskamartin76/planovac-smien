...
# --- GENEROVANIE ---
def generuj_final(m, r, fond_limit, parl_active, p_from, p_to, df_v_edit, use_extra_w, use_extra_w_viken, use_extra_w_noc, use_extra_w_noc_viken, df_db):
    ...
    abs_map = {str(row['Priezvisko']).strip(): {'d': parse_days(row['Dovolenka']), 'kz': parse_days(row['KZ']), 'v': parse_days(row['Volno'])} for _, row in df_v_edit.iterrows()}
    ...
    for d in range(1, days_count + 1):
        ...
        def get_prioritized_people(smena_target, is_75_poz=False):
            ...
            return [x for x in sorted(pool, key=lambda x: x[1])]

        # 1. Z8
        if is_workday:
            for col_f in ["Priorita_Z8", "Z8"]:
                nas_z8 = False
                for idx in [x[0] for x in get_prioritized_people('D', True)]:
        ...
        for smena in ['D', 'N']:
            # C1
            for idx in [x[0] for x in get_prioritized_people(smena)]:
                if idx in vysledky[d]['D'] or idx in vysledky[d]['N']: continue
            ...
            # LOGIKA PRE EXTRA W (Nové podmienky)
            specs_extra_w = []
            if smena == 'D':
                if is_workday and use_extra_w: specs_extra_w.append('W_EXTRA')
                if is_weekend and use_extra_w_viken: specs_extra_w.append('W_EXTRA')
            else: # Smena 'N'
                if is_workday and use_extra_w_noc: specs_extra_w.append('W_EXTRA')
                if is_weekend and use_extra_w_noc_viken: specs_extra_w.append('W_EXTRA')

            # Ostatné špeciálne pozície
            specs = ([] if smena == 'N' or not is_workday else (['TP', 'S1', 'S2', 'S3'] if parl_active and p_from <= curr_d <= p_to else []) + ['M'])
            
            for poz in (specs_extra_w + specs):
                if poz in vysledky[d][smena].values(): continue
                for idx in [x[0] for x in get_prioritized_people(smena)]:
                    if idx in vysledky[d]['D'] or idx in vysledky[d]['N']: continue
                    priez = str(df_db.loc[idx, 'Priezvisko']).strip(); ab = abs_map.get(priez, {'d':set(),'kz':set(),'v':set()})
                    p_col = 'W1' if poz == 'W_EXTRA' else poz
                    if d not in (ab['d']|ab['kz']|ab['v']) and str(df_db.loc[idx].get(p_col,'Nie')).lower() == 'áno' and moze_nastupit(idx, d, smena, poz, vysledky):
                        vysledky[d][smena][idx] = poz; hod_fond_sofar[idx] += 11.5; break

            # Priority (ZT, NB, PRIO_LIST)
            for p_n in ['ZT', 'NB'] + PRIO_LIST:
                if p_n in vysledky[d][smena].values() or (p_n == 'NB' and smena == 'D' and is_workday): continue
                for idx in [x[0] for x in get_prioritized_people(smena)]:
                    if idx in vysledky[d]['D'] or idx in vysledky[d]['N']: continue
                    priez = str(df_db.loc[idx, 'Priezvisko']).strip(); ab = abs_map.get(priez, {'d':set(),'kz':set(),'v':set()})
                    if d not in (ab['d']|ab['kz']|ab['v']) and str(df_db.loc[idx].get(p_n,'Nie')).lower() == 'áno' and moze_nastupit(idx, d, smena, p_n, vysledky):
                        vysledky[d][smena][idx] = p_n; hod_fond_sofar[idx] += 11.5; break

    # --- ZÁPIS EXCEL --- (Zachovaný vizuál)
    ...
    for i, (idx, row) in enumerate(df_db.iterrows()):
        ...
        priez = str(row['Priezvisko']).strip(); ab = abs_map.get(priez, {'d':set(), 'kz':set(), 'v':set()})
        for d in range(1, days_count + 1):
...
if uploaded_file:
    ...
    st.markdown("### Možnosti Extra W")
    ew1, ew2, ew3, ew4 = st.columns(4)
    extra_w = ew1.checkbox("Extra W (Dni)", True)
    extra_w_viken = ew2.checkbox("Extra W (Víkend)", False)
    extra_w_noc = ew3.checkbox("Extra W (Noc)", False)
    extra_w_noc_viken = ew4.checkbox("Extra W (Noc + Víkend)", False)
    ...
    df_v_edit = st.data_editor(df_v.sort_values(by=['Priezvisko', 'Meno']), use_container_width=True, key="volno_edit", 
                               column_config={"Priezvisko": st.column_config.Column(disabled=True), "Meno": st.column_config.Column(disabled=True)})
    
    if st.button("🚀 GENEROVAŤ PLÁN", type="primary", use_container_width=True):
        xlsx, name = generuj_final(mes, 2026, fon, parl, p_od, p_do, df_v_edit, extra_w, extra_w_viken, extra_w_noc, extra_w_noc_viken, df_db)
