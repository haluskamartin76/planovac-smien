...
def get_prioritized_people(df_db, curr_d, smena_target, hod_fond_sofar, fond_limit, is_75_poz=False):
    ...
    # VRACIA LEN INDEXY (opravené triedenie)
...
def generuj_final_streamlit(m, r, fond_limit, parl_active, p_from, p_to, df_volno_edited, use_extra_w, df_db):
    output = io.BytesIO()
    # Použijeme priamo xlsxwriter pre plnú kontrolu nad hárkami
    ...
    # Výpočet hodín na začiatku mesiaca
    for idx in df_db.index:
    ...
    # Samotná logika priraďovania (tvoja pôvodná)
    ...
    for i, (idx, row) in enumerate(df_db.iterrows()):
        ...
        # Komplexné Excel vzorce na výpočet fondu
...
if os.path.exists(DB_FILENAME):
    ...
    with t2:
        st.subheader("Editácia personálu")
        ...
        if st.button("💾 ULOŽIŤ PERSONÁL NA GITHUB"):
            if push_to_github(df_db_edit, df_v_raw): st.success("Dáta úspešne uložené na GitHub!")
            else: st.error("Nepodarilo sa uložiť na GitHub.")
    ...
    with t1:
        ...
        if st.button("💾 ULOŽIŤ ABSENCIE NA GITHUB"):
            if push_to_github(df_db_edit, df_v_edit): st.success("Absencie úspešne uložené na GitHub!")
            else: st.error("Nepodarilo sa uložiť na GitHub.")
    ...
    st.error(f"Súbor {DB_FILENAME} nenájdený v koreňovom adresári.")
    st.info("Nahrajte súbor databaza_pozicii.xlsx do vášho GitHub repozitára.")
