...
if df_db_raw is not None:
    ...
    st.subheader("📅 Zadanie absencií (Dovolenky, KZ, Voľno)")
    v_data = {}
    
    # Zobrazenie v mriežke pre lepšiu prehľadnosť
    ...
    for i, (idx, row) in enumerate(df_db.iterrows()):
        with abs_cols[i % 3]:
            with st.container(border=True):
                ...
                # Predvyplnenie hodnôt ak existujú
                vd_def, vk_def = "", ""
    ...
    st.error(f"Súbor {DB_FILENAME} nebol nájdený. Nahrajte ho do koreňového adresára na GitHub.")
